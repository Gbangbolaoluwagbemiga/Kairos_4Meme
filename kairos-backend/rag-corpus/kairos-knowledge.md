...
This document is indexed for retrieval-augmented answers about Kairos (Four.meme AI Sprint build), on-chain agent payments, and the agentic ecosystem.

---

## What Kairos Is

Kairos is an **AI agent marketplace** built for the **Four.meme AI Sprint**. Users chat with an AI orchestrator (Groq) that routes queries to specialist agents. Each agent call triggers a **real on-chain payment receipt** — agents earn for their work, pay sub-agents for coordination, and build on-chain reputation.

### Key Differentiators
- **Native BNB micropayments** — Each agent call settles on-chain
...
- **Auditable** — All payments visible on-chain with clickable tx hashes
...
| Chain Scout | `chain-scout` | EVM account facts (balance, nonce, contract detection) | (on-chain) |
...
```
User query → Orchestrator → Agent A  →  BNB (treasury → oracle)
                          → Agent B  →  BNB (treasury → news)
```
...
```
Agent A (oracle) → Agent B (news)  →  BNB A2A payment
```
...
## EVM Fundamentals (Quick)

- **Accounts**: Externally Owned Accounts (EOAs) and contracts (0x…)
- **Value transfer**: native token via transaction `value` (displayed as BNB in this demo)
- **Units**: wei for on-chain integers; human display in ether units (BNB)
- **RPC**: JSON-RPC endpoint (`FOURMEME_RPC_URLS` / `FOURMEME_RPC_URL`), chain id `97` for BNB testnet
...
| `FOURMEME_RPC_URLS` / `FOURMEME_RPC_URL` | Yes | BNB testnet RPC URL(s) |
| `FOURMEME_CHAIN_ID` | Yes | `97` |
| `FOURMEME_TREASURY_PRIVATE_KEY` | Yes | Treasury private key (0x…) |
...
For questions about Kairos features, on-chain payments, or deployment:
...
Kairos is rebuilt for the **Four.meme AI Sprint 2026**:
