...
/**
 * Backfill tx_hash for an existing logical_id row (used when treasury payment lands after the first response).
 */
export async function updateQueryLogTxHash(logicalId: string, txHash: string): Promise<boolean> {
    if (!supabase) return false;
    if (!logicalId || !txHash) return false;
    try {
        ...
        if (error) {
            console.error('[Supabase] Failed to backfill tx_hash:', error);
            return false;
        }
        return true;
    } catch (e: any) {
        console.error('[Supabase] updateQueryLogTxHash exception:', e?.message || e);
        return false;
    }
}
...
export async function getAgentTreasuryBalance(agentId: string): Promise<number> {
    if (!supabase) return 0;
    const DEFAULT_PER_CALL = Number(process.env.KAIROS_DEFAULT_AGENT_PRICE_BNB || "0.0005") || 0.0005;
    const DEFAULT_A2A = Number(process.env.KAIROS_A2A_PRICE_BNB || "0.00025") || 0.00025;
    ...
    if (error) {
        ...
        if (error.message?.includes('does not exist')) {
            const { count } = await supabase
                ...
                .select('*', { count: 'exact', head: true })
                .eq('agent_id', agentId);
            return (count || 0) * DEFAULT_PER_CALL;
        }
    ...
    const rowAmt = (row: { nominal_usd?: unknown; direction?: string | null }) => {
        ...
        // Missing/zero amount: debits default to A2A price; credits default to per-call price.
        return d === 'debit' ? DEFAULT_A2A : DEFAULT_PER_CALL;
    };
