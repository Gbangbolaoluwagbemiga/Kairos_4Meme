...
import { Copy, ThumbsUp, ThumbsDown, Check, User, Bot, ExternalLink, Image as ImageIcon, BookOpen, ArrowRight, Globe } from 'lucide-react';
import type { RagSourceRef, WebSourceRef } from '@/contexts/ChatContext';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { useState, useEffect } from 'react';
import { useWallet } from '@/contexts/WalletContext';
import { PaymentPulse } from './PaymentPulse';
import { txUrl } from '@/lib/fourmeme';
...
export function ChatMessage({ id, content, isUser, timestamp, imagePreview, agentsUsed, txHashes, a2aPayments, partial, ragSources, webSources, onContinue }: ChatMessageProps) {
  ...
  return (
    <div className={cn('flex gap-3 animate-fade-in-up max-w-3xl mx-auto w-full px-4', isUser && 'flex-row-reverse')}>
      ...
      <div className={cn('flex-1 min-w-0 space-y-1.5', isUser && 'items-end flex flex-col')}>
        ...
        {!isUser && (webSources?.length || 0) > 0 && (
          <div className="flex flex-wrap gap-2 mb-1">
            <span
              className="inline-flex items-center gap-1 py-1 px-2 rounded-lg text-[10px] font-semibold uppercase tracking-wide bg-sky-500/10 text-sky-300 border border-sky-500/25"
              title="This reply used web search snippets (when configured)"
            >
              <Globe className="w-3 h-3 opacity-90" />
              Web · {webSources!.length} source{webSources!.length > 1 ? 's' : ''}
            </span>
          </div>
        )}
            ...
            {agentsUsed.map((agentId) => {
              ...
              const labelRow = (
                <>
                  ...
                  <span className="text-[9px] bg-sky-500/20 px-1 rounded text-sky-400 border border-sky-500/30">BNB</span>
                </>
              );
            ...
            {a2aPayments.map((p, i) => {
              ...
              return (
                <a
                  ...
                  title={`Agent-to-Agent: ${p.from} paid ${p.to} ${p.amount} BNB for sub-task coordination`}
                >
                  <span style={{ color: fromMeta.dot }}>{fromMeta.label}</span>
                  <ArrowRight className="w-2.5 h-2.5 opacity-50" />
                  <span style={{ color: toMeta.dot }}>{toMeta.label}</span>
                  <span className="ml-0.5 text-emerald-400/70">{p.amount} BNB</span>
        ...
        {!isUser && webSources && webSources.length > 0 && (
          <div className="mt-1">
            <button
              type="button"
              onClick={() => setShowWeb((v) => !v)}
              className="inline-flex items-center gap-1.5 text-[10px] font-medium text-muted-foreground/80 hover:text-sky-300/90 transition-colors"
            >
              <Globe className="w-3 h-3 opacity-70" />
              {showWeb ? 'Hide web sources' : `${webSources.length} web source${webSources.length > 1 ? 's' : ''}`}
            </button>
            {showWeb && (
              <ul className="mt-1.5 space-y-1.5 pl-0.5 border-l border-sky-500/20 ml-1 py-1">
                {webSources.map((s, i) => (
                  <li key={`${s.url || s.title}-${i}`} className="text-[10px] text-muted-foreground/90 leading-snug list-none">
                    <span className="text-sky-400/90 font-medium">[{s.provider}{s.liveWeb ? '' : ':offline'}]</span>{' '}
                    ...
                    <span className="block mt-0.5 text-muted-foreground/70">{s.content}</span>
                  </li>
                ))}
              </ul>
            )}
