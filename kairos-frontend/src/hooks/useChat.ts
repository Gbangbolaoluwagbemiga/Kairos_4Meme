...
/**
 * 🛠️ Kairos: Four.meme Sprint Chat Hook
 */
...
export function useChat() {
  ...
  const sendMessage = useCallback(async (content: string, imageData?: ImageData) => {
    ...
    try {
      ...
      const responseTone = (localStorage.getItem('kairos_tone') || 'trader').toLowerCase();
      const response = await fetch(`${API_BASE_URL}/query`, {
        ...
        body: JSON.stringify({
          ...
          userAddress: address,
          requestId,
          tone: responseTone === 'beginner' ? 'beginner' : responseTone === 'research' ? 'research' : 'trader',
